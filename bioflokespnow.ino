...
void captureAndPublish() {
  ...
  // Analyze center portion of image
  int centerW = fb->width / 2;
  int centerH = fb->height / 2;
  int startX = (fb->width - centerW) / 2;
  int startY = (fb->height - centerH) / 2;

  uint32_t sumR = 0, sumG = 0, sumB = 0, pixelCount = 0;

  for (int y = startY; y < startY + centerH; y += 4) {
    for (int x = startX; x < startX + centerW; x += 4) {
      int pos = (y * fb->width + x) * 2;
      if (pos + 1 < fb->len) {
        uint16_t pixel = (fb->buf[pos] << 8) | fb->buf[pos + 1];
...
float calculateMCCI(uint8_t r, uint8_t g, uint8_t b) {
  if ((g - b) == 0) return 0.0; 

  float mcci = (float)(r - b) / (float)(g - b);
  if (mcci < 0.0) mcci = 0.0;
  if (mcci > 5.0) mcci = 5.0;
  return mcci;
}
